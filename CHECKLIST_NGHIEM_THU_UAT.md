...
| **7** | **Bảo mật, Sao lưu & Bàn giao Kỹ thuật** | - Rate limiting chống brute force & spam OTP<br/>- Security headers (Helmet)<br/>- Khóa chặt CORS domain whitelist<br/>- Bất biến Audit Log ở cấp Engine PostgreSQL<br/>- Script sao lưu tự động cho Linux/Docker (.sh) & Windows (.bat) có mốc tháng 12 tháng<br/>- Diễn tập khôi phục dữ liệu có biên bản<br/>- Bảo vệ OpenAPI/Swagger UI tại `/api/docs` qua Basic Auth<br/>- Đầy đủ tài liệu quản trị, hướng dẫn sử dụng và văn bản chốt điều kiện với HVE | - Rate limit 1000/60s toàn cục, login (5 lần/p) & OTP (3 lần/p)<br/>- Helmet bảo vệ header<br/>- Fail-closed triệt để: không có bí mật fallback hardcode<br/>- Script `db_security_hardening.sql` REVOKE DELETE AuditLog trên DB `hve_db`<br/>- Script backup hỗ trợ cả rolling 30 ngày & 12 bản chụp tháng trong 1 năm<br/>- Swagger `/api/docs` khóa Basic Auth an toàn<br/>- Đã lập `BIEN_BAN_TEST_RESTORE.md`<br/>- Đã lập `HUONG_DAN_QUAN_TRI.md`<br/>- Đã lập `HUONG_DAN_SU_DUNG.md`<br/>- Đã lập `VAN_BAN_XAC_NHAN_CHOT_HVE.md` | ✅ **ĐẠT** |
