...
describe('DashboardService role scopes', () => {
  ...
  it('gives CEO company-wide metrics and project statistics', async () => {
    ...
    prismaMock.task.groupBy.mockResolvedValue([
      { projectId: 1, status: 'Đang làm', _count: { _all: 1 } },
      { projectId: 1, status: 'Hoàn thành', _count: { _all: 1 } },
    ]);
    prismaMock.document.groupBy.mockResolvedValue([
      { status: 'Chờ duyệt', _count: { _all: 1 } },
      { status: 'Đã duyệt', _count: { _all: 1 } },
    ]);
    ...
    expect(result.metrics.tasks.active).toBe(1);
  ...
  it('scopes a department head to the department', async () => {
    prismaMock.document.findMany
      .mockResolvedValueOnce([
        { id: 2, type: 'proposal', status: 'Chờ duyệt', dataJson: {} },
      ])
  ...
  it('uses cache per user, department and complete role set', async () => {
    ...
    const callsAfterFirstRequest =
      prismaMock.document.findMany.mock.calls.length;
    await service.getDashboardData(user);
    expect(prismaMock.document.findMany).toHaveBeenCalledTimes(
